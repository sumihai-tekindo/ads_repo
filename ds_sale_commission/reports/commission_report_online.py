...
class sale_commission_online_xls_parser(report_sxw.rml_parse):
	
	def __init__(self, cr, uid, name, context):
		super(sale_commission_online_xls_parser, self).__init__(cr, uid, name, context=context)
	...
	def _get_grouped_data(self,objects,user_id):
		...
		res = {}
		for data in datas:
			wholesale= data.rule_type == 'whs_on' and data.amount_total or 0.0
			c_wholesale= data.rule_type == 'whs_on' and data.commission_amount or 0.0
			retail = data.rule_type =='ret_on' and data.amount_total or 0.0
			c_retail = data.rule_type =='ret_on' and data.commission_amount or 0.0
...
class sale_commission_online_xls(report_xls):
	
	def __init__(self, name, table, rml=False, parser=False, header=True,
				 store=False):
		super(sale_commission_online_xls, self).__init__(
	...
	def generate_xls_report(self, _p, _xs, data, objects, wb):
		...
		datadummy = _p.get_user_ids(objects)
		if datadummy:
			for user_id in datadummy:
				...
				ws.write_merge(0,0,0,4,"Laporan Komisi Penjualan Online",title_style_center)
...
sale_commission_online_xls('report.sale.commission.online.xls', 'commission.compute.line.detail', parser=sale_commission_online_xls_parser)