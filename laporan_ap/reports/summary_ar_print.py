...
class summary_ar_xls_parser(report_sxw.rml_parse):
    
    def __init__(self, cr, uid, name, context):
        super(summary_ar_xls_parser, self).__init__(cr, uid, name,
                                                         context=context)
        self.context = context


class summary_ar_xls(report_xls):
    
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(summary_ar_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        ws.write_merge(0,0,0,4,"SUMMARY AR",title_style_center)
        ...
        headers = ["No","Nama Customer","Total Invoice","Payment","Outstanding"]
...
summary_ar_xls('report.summary.ar.xls', 'account.invoice', parser=summary_ar_xls_parser)
