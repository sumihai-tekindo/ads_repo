...
class ReportAgedPartnerBalance(models.AbstractModel):
	...
	def _get_partner_move_lines(self, form, account_type, date_from, target_move):
		...
		self.total_account = []
		cr = self.env.cr
		company_id=form.get('used_context',False) and form.get('used_context').get('company_id',self.env.user.company_id.id)
		company_data = self.env['res.company'].search([('id','=',company_id)])
		precision = company_data and company_data.currency_id.rounding or self.env.user.company_id.currency_id.rounding
		user_company = company_id or self.env.user.company_id.id
		...
		for partner in partners:
			...
			if not float_is_zero(values['direction'], precision_rounding=precision):
			...
			for i in range(5):
				...
				if not float_is_zero(values[str(i)], precision_rounding=precision):
					at_least_one_amount = True
	...
	def render_html(self, data):
		...
		return self.env['report'].with_context({'company_id':company_id}).render('account.report_agedpartnerbalance', docargs)