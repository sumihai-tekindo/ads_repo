...
class laporan_sales_xls_parser(report_sxw.rml_parse):
    
    def __init__(self, cr, uid, name, context):
        super(laporan_sales_xls_parser, self).__init__(cr, uid, name,
                                                         context=context)
        self.context = context


class laporan_sales_xls(report_xls):
    
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(laporan_sales_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        ws.write_merge(0,0,0,14,"LAPORAN SALES",title_style_center)
        ...
        headers = ["No","Nama Toko","Penjualan Wholesale","Komisi Wholesale","Penjualan Retail","Komisi Retail"]
        row+=1
        col = 0
        no=1
        for head in headers:
            ws.write_merge(row,row+1,col,col,head,bold_border_all)
            col+=1
        ...
        for rec in objects:
            ...
            ws.write(row,2,rec.debit,normal_style_float)
            ws.write(row,3,rec.debit,normal_style_float)
            ws.write(row,4,rec.debit,normal_style_float)               #ini belum ambil datanya
            ws.write(row,5,rec.debit,normal_style_float)
            
            max_len[0]=len(str(no))+3 > max_len[0] and len(str(no))+3 or max_len[0]
            max_len[1]=len(str(rec.partner_id)) > max_len[1] and len(str(rec.partner_id)) or max_len[1]
            max_len[2]=len(str(rec.debit)) > max_len[2] and len(str(rec.debit)) or max_len[2]
            max_len[3]=len(str(rec.debit)) > max_len[3] and len(str(rec.debit)) or max_len[3]
            max_len[4]=len(str(rec.debit)) > max_len[4] and len(str(rec.debit)) or max_len[4]
            max_len[5]=len(str(rec.debit)) > max_len[5] and len(str(rec.debit)) or max_len[5]
...
laporan_sales_xls('report.laporan.sales.xls', 'account.move.line', parser=laporan_sales_xls_parser)
