...
class laporan_comm_online_xls_parser(report_sxw.rml_parse):
    
    def __init__(self, cr, uid, name, context):
        super(laporan_comm_online_xls_parser, self).__init__(cr, uid, name,
                                                         context=context)
    ...
    def _group_data(self,objects):
        commissions = {}
        for comm in objects:
            rule_type = (comm.rule_type=='whs_on' and 'wholesale') or (comm.rule_type=='ret_on' and 'retail') or False
            amt_whs = comm.rule_type=='whs_on' and comm.amount_untaxed or 0.0
            comm_whs = comm.rule_type=='whs_on' and comm.commission_amount or 0.0
            amt_ret = comm.rule_type=='ret_on' and comm.amount_untaxed or 0.0 
            comm_ret = comm.rule_type=='ret_on' and comm.commission_amount or 0.0
            if rule_type:
                if comm.sale_user_id not in commissions:
                    commissions.update({comm.sale_user_id:{comm.journal_id:{'wholesale':{'amount':amt_whs,'commission':comm_whs},'retail':{'amount':amt_ret,'commission':comm_ret}}}})
                else:
                    if comm.journal_id not in commissions[comm.sale_user_id]:
                        commissions[comm.sale_user_id].update({comm.journal_id:{'wholesale':{'amount':amt_whs,'commission':comm_whs},'retail':{'amount':amt_ret,'commission':comm_ret}}})
                    else:
                        if rule_type not in commissions[comm.sale_user_id][comm.journal_id]:
                            commissions[comm.sale_user_id][comm.journal_id]['wholesale'].update({'amount':amt_whs,'commission':comm_whs})
                            commissions[comm.sale_user_id][comm.journal_id]['retail'].update({'amount':amt_ret,'commission':comm_ret})
                        else:
                            amt_whs+=commissions[comm.sale_user_id][comm.journal_id]['wholesale']['amount']
                            comm_whs+=commissions[comm.sale_user_id][comm.journal_id]['wholesale']['commission']
                            amt_ret+=commissions[comm.sale_user_id][comm.journal_id]['retail']['amount']
                            comm_ret+=commissions[comm.sale_user_id][comm.journal_id]['retail']['commission']
                            commissions[comm.sale_user_id][comm.journal_id]['wholesale'].update({'amount':amt_whs,'commission':comm_whs})
                            commissions[comm.sale_user_id][comm.journal_id]['retail'].update({'amount':amt_ret,'commission':comm_ret})
        return commissions

class laporan_comm_online_xls(report_xls):
    
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(laporan_comm_online_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        ws.write_merge(0,0,0,6,"LAPORAN KOMISI ONLINE",title_style_center)
        ...
        headers = ["No","Sales Person","Toko","Penjualan Wholesale","Komisi Wholesale","Penjualan Retail","Komisi Retail"]
        ...
        for sales in grouped_data:  
            ...
            start_row = row
            for journal in grouped_data[sales]:
                ws.write(row,2,journal.name,normal_style)
                ws.write(row,3,grouped_data[sales][journal]['wholesale']['amount'],normal_style_float)
                ws.write(row,4,grouped_data[sales][journal]['wholesale']['commission'],normal_style_float)
                ws.write(row,5,grouped_data[sales][journal]['retail']['amount'],normal_style_float)              
                ws.write(row,6,grouped_data[sales][journal]['retail']['commission'],normal_style_float)
                ...
                max_len[2]=len(str(journal.name)) > max_len[2] and len(str(journal.name)) or max_len[2]
                max_len[3]=len(str(grouped_data[sales][journal]['wholesale']['amount'])) > max_len[3] and len(str(grouped_data[sales][journal]['wholesale']['amount'])) or max_len[3]
                max_len[4]=len(str(grouped_data[sales][journal]['wholesale']['commission'])) > max_len[4] and len(str(grouped_data[sales][journal]['wholesale']['commission'])) or max_len[4]
                max_len[5]=len(str(grouped_data[sales][journal]['retail']['amount'])) > max_len[5] and len(str(grouped_data[sales][journal]['retail']['amount'])) or max_len[5]
                max_len[6]=len(str(grouped_data[sales][journal]['retail']['commission'])) > max_len[6] and len(str(grouped_data[sales][journal]['retail']['commission'])) or max_len[6]
            ...
            # print "fxxxxxxxxxxxxxxx",formula1
            row+=2
        for x in range(0,7):
...
laporan_comm_online_xls('report.laporan.comm.online.xls', 'commission.compute.line.detail', parser=laporan_comm_online_xls_parser)
