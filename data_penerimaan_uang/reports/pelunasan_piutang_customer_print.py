...
class pelunasan_piutang_customer_xls_parser(report_sxw.rml_parse):
    
    def __init__(self, cr, uid, name, context):
        super(pelunasan_piutang_customer_xls_parser, self).__init__(cr, uid, name,
                                                         context=context)
        self.context = context


class pelunasan_piutang_customer_xls(report_xls):
    
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(pelunasan_piutang_customer_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for group in grouping.keys():
            ...
            ws.write_merge(0,0,0,2,"PELUNASAN PIUTANG CUSTOMER",title_style_center)
            ...
            headers = ["Nama Customer","Pembayaran","Bank"]
            ...
            for rec in grouping.get(group,[]):    
                ws.write(row,0,rec.partner_id.name,normal_style)
                ws.write(row,1,rec.credit,normal_style_float)
                ws.write(row,2,rec.journal_id.name,normal_style)
                
                max_len[0]=len(str(rec.partner_id.name)) > max_len[0] and len(str(rec.partner_id.name)) or max_len[0]
                max_len[1]=len(str(rec.credit)) > max_len[1] and len(str(rec.credit)) or max_len[1]
                max_len[2]=len(str(rec.journal_id.name)) > max_len[2] and len(str(rec.journal_id.name)) or max_len[2]
                
                row+=1
            for x in range(0,3):
                ws.col(x).width=max_len[x]*256
                
            ws.write(row+1,0,"TOTAL",title_style)
            #SUM('BCA 270.302.7211'!B9:B103)
            ws.write(row+1,1,xlwt.Formula("SUM('"+group.name+"'!$B$9:$B$"+str(row)+")"),subtotal_style2)
        
        
        
pelunasan_piutang_customer_xls('report.pelunasan.piutang.customer.xls', 'account.move.line', parser=pelunasan_piutang_customer_xls_parser)
