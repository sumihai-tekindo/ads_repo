...
class laporan_penjualan_xls_parser(report_sxw.rml_parse):
    
    def __init__(self, cr, uid, name, context):
        super(laporan_penjualan_xls_parser, self).__init__(cr, uid, name,
                                                         context=context)
...
class laporan_penjualan_xls(report_xls):
    
    def __init__(self, name, table, rml=False, parser=False, header=True,
                 store=False):
        super(laporan_penjualan_xls, self).__init__(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        ws = wb.add_sheet("Laporan Penjualan Online")
        ...
        ws.write_merge(0,0,0,4,"LAPORAN PENJUALAN ONLINE",title_style_center)
        ...
        headers = ["No","Nama Toko Online","Penjualan Wholesale","Penjualan Retail","Total Penjualan Wholesale & retail"]
        ...
        for rec in invoices:
            ...
            ws.write(row,2,invoices[rec]['wholesale'],normal_style_float)
            ws.write(row,3,invoices[rec]['retail'],normal_style_float)
            ws.write(row,4,invoices[rec]['wholesale']+invoices[rec]['retail'],normal_style_float)
...
laporan_penjualan_xls('report.laporan.penjualan.xls', 'account.invoice', parser=laporan_penjualan_xls_parser)
